...
class ComplaintsApi(val defBasePath: String = "https://www.callcontrol.com",
   ...
   * @return Complaints
   */
  def complaintsComplaints (phoneNumber: String) : Option[Complaints] = {
    // create path and map variables
    val path = "/api/2015-11-01/Complaints/{phoneNumber}".replaceAll("\\{format\\}","json").replaceAll("\\{" + "phoneNumber" + "\\}",apiInvoker.escape(phoneNumber))
    ...
    try {
      apiInvoker.invokeApi(basePath, path, "GET", queryParams.toMap, formParams.toMap, postBody, headerParams.toMap, contentType) match {
        case s: String =>
           Some(ApiInvoker.deserialize(s, "", classOf[Complaints]).asInstanceOf[Complaints])
